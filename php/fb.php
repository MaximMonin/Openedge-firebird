...
// Create XML TempTable from sql query
// php fb.php "select * from adm_firmy"
// php fb.php "select * from gm_zest_mag_on_day (1,34,'07.07.2013',0)" WhRest > 1.xml
// php fb.php "select * from adm_firmy" admfirmy UseTranslit > 1.xml"
// first argument  = sql query
// second argument = output tablename. Can be ommited.
...
$col_info = ibase_field_info($sth, 0);    

$tablename = $col_info['relation'];
// change default tablename
if ($argc > 2) {$tablename = $argv[2];}


echo "<?xml version=\"1.0\"?>\n";
echo "<", $tablename, " xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">\n";
echo "  <xsd:schema xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns=\"\" xmlns:prodata=\"urn:schemas-progress-com:xml-prodata:0001\">\n";
echo "    <xsd:element name=\"", $tablename, "\" prodata:proTempTable=\"true\">\n";
echo "      <xsd:complexType>\n";
echo "        <xsd:sequence>\n";
echo "          <xsd:element name=\"", $tablename, "Row\" minOccurs=\"0\" maxOccurs=\"unbounded\">\n";
echo "            <xsd:complexType>\n";
echo "              <xsd:sequence>\n";
